...
import type { Label, WebhookEvent } from '@octokit/webhooks-types';
import type { Metric, Tags } from './metrics-types.js';
  ...
  if ('pull_request' in payload && payload.pull_request) {
    return {
      ...
      merged:
        ...
        'merged' in payload.pull_request &&
...
function generateDurationSeconds(
  params: Parameters,
  tags: Tags,
): Metric | undefined {
  ...
  if (closedAt) {
    ...
    return {
      ...
      tags,
      value:
        durationSeconds < 1 ? durationSeconds : Math.round(durationSeconds),
    };
  }
}
...
export function generateMetrics(
  ...
  const labels = params.labels
    .filter((o) => labelWhitelist[o.name])
    .map((o) => o.name);
...
export async function main() {
  ...
  const labelWhitelist = core.getMultilineInput('tag-labels').reduce(
    (a, e) => {
      a[e] = true;
      return a;
    },
